  ...
  render() {
      ...
      return (
        <div style={{position: 'fixed', zIndex : 10000, backgroundColor: 'rgba(155,155,155, 0)', 'top':'0%', 'left':'0%', 'height':'100%',}} >
          <button onClick={(e) => this.props.closePopup(e)}  style={{"width":"100%", "height":"100%", position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, margin: 'auto', backgroundColor: 'rgba(155,155,155, 0.8)'}}></button>
          <PopUpAnimation pose={this.state.appear ? "hovered" : "idle"} style={{background: 'white', position : 'fixed', width:viewWidth, overflow:"hidden"}}>
            <div className='menu' style={{width:'100%', height:"5vh", color:'black', backgroundColor:'rgba(248,248,248,1)', display:'flex', justifyContent:'space-between'}}>
              <div style={{width:50, textAlign:'left', marginLeft:10, display:'flex', alignItems:'center', fontSize:fontSize}}>
                {this.props.Movie.length>1?
                  <div style={{height:30, width:30, display:"flex", justifyContent:"center", alignItems:"center", marginLeft:10, borderRadius:"50%", backgroundColor:this.state.hoverPrevious ? "rgba(100,100,100,0.3)" : "rgba(100,100,100,0)" }} onClick={this.previousMovie} onMouseEnter={this.enterPrevious} onMouseLeave={this.leavePrevious}>
