...
class FavouritesComponent extends React.Component {
  ...
  componentDidMount = () => {
    var that = this
    window.$("#displayFavourites").bind('mousewheel DOMMouseScroll', function(event){
  ...
  render(){
    ...
    return(
      <div style={{display:"flex", flexDirection:"column", backgroundColor:theme.background.element1.interior, overflow:"auto", height:"100%", paddingTop:25}} id="displayFavourites">
        <h5>MY FAVOURITES</h5>
        <div style={{width: '100%', display: 'flex', flexDirection: 'row', flexWrap:'wrap',  overflowY: 'auto', marginTop:25, }}>

          {this.props.accountState.favourites.length>0 ?
            this.props.accountState.favourites.map((movie) => (
              this.renderMovies(movie)
            ))
            :
            <div>You do not have favourites moovies in your list</div>
          }
        </div>
      </div>
    )
...
const Favourites = connect(mapStateToProps, mapDispatchToProps)(FavouritesComponent);
